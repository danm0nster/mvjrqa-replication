...
df <- read_csv("results/mvjrqa.csv") |> 
  select(-c(eye2d_len1, eye2d_len2, eye2d_len3, eye3d_len1, eye3d_len2, eye3d_len3))

# Reshapes data from wide to long: splits column names into "Eye_Type" (2d/3d) and measurement type
df2 <- df %>%
  pivot_longer(
    cols = c(eye2d_JRR, eye3d_JRR, eye2d_DET, eye3d_DET, eye2d_LAM, eye3d_LAM,
             eye2d_DENTR, eye3d_DENTR, eye2d_RR1, eye3d_RR1, eye2d_RR2, eye3d_RR2,
             eye2d_RAD1, eye3d_RAD1, eye2d_RAD2, eye3d_RAD2, eye2d_file, eye3d_file),  
